      ...
      <table className="andean-table" style={{ fontSize: "0.9rem", width: "100%" }}>
        <thead>
          <tr>
            ...
            {canWrite && <th style={{ textAlign: "right", width: "100px" }}>Acciones</th>}
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={i} style={{ borderBottom: "1px solid rgba(0,0,0,0.03)" }}>
              {columns.map((c) => (
                <td key={c.header}>{c.accessor(row)}</td>
              ))}
              {canWrite && (
                <td style={{ textAlign: "right" }}>
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.4rem" }}>
                    {!hideEdit && (
                      <button onClick={() => onEdit(row)} style={btnEdit} title="Editar">
                        <Pencil size={14} />
                      </button>
                    )}
                    <button onClick={() => onDelete(row)} style={btnDanger} title="Eliminar">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
...
function FormModal({
  ...
  return (
    <div style={modalOverlay} onClick={onClose}>
      <div style={modalBox} onClick={(e: MouseEvent) => e.stopPropagation()}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      >
        ...
        >
          <CalendarDays color="var(--color-secondary)" /> Planes y Periodos Académicos
        </h2>
        <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
          Configura los planes académicos, sus períodos (bimestres, trimestres) y asigna cursos.
        </p>
      </div>
      ...
      >
        <div
          style={{
            display: "flex",
            borderBottom: "1px solid rgba(45, 55, 63, 0.08)",
            padding: "0 1rem",
          }}
        >
          <button onClick={() => setTab("plans")} style={tabStyle(tab === "plans")}>
            Planes Académicos
          </button>
          <button onClick={() => setTab("terms")} style={tabStyle(tab === "terms")}>
            Períodos
          </button>
          <button onClick={() => setTab("plan-courses")} style={tabStyle(tab === "plan-courses")}>
            Cursos en Plan
          </button>
        </div>
...
function TabPlans({ canWrite }) {
  ...
  const fetch_ = async () => {
    setLoading(true);
    try {
      const d = await apiFetch(`${API}/academic/plans`);
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", duration_type: "annual", period_type: "bimester", period_count: "" });
    setModal(true);
  };
  const openEdit = (row) => {
    setEditing(row);
    setForm({
      name: row.name,
      duration_type: row.duration_type || "annual",
      period_type: row.period_type || "bimester",
      period_count: row.period_count ?? "",
    });
    setModal(true);
  };
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar plan "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/plans/${row.plan_id}`, { method: "DELETE" });
      fetch_();
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const body = {
        ...
        period_count: form.period_count ? Number(form.period_count) : null,
      };
      if (editing) {
        await apiFetch(`${API}/platform/academic/plans/${editing.plan_id}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch(`${API}/platform/academic/plans`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
  ...
  if (loading)
    return (
      <div style={{ textAlign: "center", padding: "2rem" }}>
        <Loader2
          size={28}
          style={{ animation: "spin 1s linear infinite", color: "var(--color-secondary)" }}
        />
      </div>
    );
    ...
    <>
      ...
      >
        <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.85rem" }}>
          Planes académicos globales que definen la estructura temporal del año escolar.
        </p>
        {canWrite && (
      ...
      >
        <div style={{ marginBottom: "1rem" }}>
          ...
          <input
            ...
            placeholder="Ej: Plan Anual Bimestral"
          />
        </div>
        <div
          style={{
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setForm({ ...form, period_count: e.target.value })}
            placeholder="Ej: 4"
          />
        </div>
      </FormModal>
...
function TabTerms({ canWrite }) {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ plan_id: plans[0]?.plan_id || "", name: "", order: "" });
    setModal(true);
  };
  const openEdit = (row) => {
    setEditing(row);
    setForm({ plan_id: row.plan_id, name: row.name, order: row.order ?? "" });
    setModal(true);
  };
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar período "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/terms/${row.term_id}`, { method: "DELETE" });
      fetch_();
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await apiFetch(`${API}/platform/academic/terms/${editing.term_id}`, {
          method: "PUT",
          body: JSON.stringify({ name: body.name, order: body.order }),
        });
      } else {
        await apiFetch(`${API}/platform/academic/terms`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
    ...
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
          <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.85rem" }}>
            Bimestres, trimestres u otros períodos de cada plan.
          </p>
          <select
            value={filterPlan}
...
function TabPlanCourses({ canWrite }) {
  ...
  const openCreate = () => {
    setForm({
      ...
      grade_course_id: gradeCourses[0]?.grade_course_id || "",
    });
    setModal(true);
  };
  const handleDelete = async (row) => {
    if (!confirm("¿Desasignar este curso del plan?")) return;
    try {
      await apiFetch(`${API}/platform/academic/plan-courses/${row.plan_course_id}`, {
        method: "DELETE",
      });
      fetch_();
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await apiFetch(`${API}/platform/academic/plan-courses`, {
        method: "POST",
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
          <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.85rem" }}>
            Vinculación entre planes y cursos de grado.
          </p>
          <select
            value={filterPlan}
            onChange={(e) => setFilterPlan(e.target.value)}
      ...
      >
        ...
        <div>
          ...
          <select
            required
            style={inputStyle}
            value={form.grade_course_id}
            onChange={(e) => setForm({ ...form, grade_course_id: e.target.value })}
          >
            <option value="" disabled>
              Seleccionar curso
            </option>
            {gradeCourses.map((gc) => (
              <option key={gc.grade_course_id} value={gc.grade_course_id}>
                {gc.name}
              </option>
            ))}
          </select>
