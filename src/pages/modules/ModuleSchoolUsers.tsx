...
export default function ModuleSchoolUsers({ schoolId, initialFilter = "" }) {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRoles(roleItems);
        setFormData((prev) => {
          if (roleItems.length > 0 && !prev.role_id) {
            return { ...prev, role_id: roleItems[0].id };
          }
          return prev;
        });
      }
  ...
  const openCreateModal = (mode = "generic") => {
    ...
    if (mode === "profesor") {
      ...
      const r = roles.find(
        (x) =>
          x.name.toLowerCase().includes("alumno") || x.name.toLowerCase().includes("estudiante"),
      );
      if (r) targetRoleId = r.id;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (payload.birth_date) {
      payload.birth_date = new Date(String(payload.birth_date)).toISOString();
    } else {
      ...
      >
        ...
        <div style={{ display: "flex", gap: "0.8rem" }}>
          {(!initialFilter || initialFilter === "") && (
            <button
              onClick={() => openCreateModal("generic")}
              style={{
          ...
          {initialFilter === "profesor" && (
            <button
              onClick={() => openCreateModal("profesor")}
              style={{
          ...
          {initialFilter === "alumno" && (
            <button
              onClick={() => openCreateModal("alumno")}
              style={{
      ...
      >
        ...
        <div style={{ position: "relative" }}>
          <select
            ...
            disabled={Boolean(initialFilter)}
            style={{
              padding: "0.5rem 1rem",
              borderRadius: "8px",
              border: "1px solid rgba(45, 55, 63, 0.1)",
                ...
                {filteredUsers.map((u) => (
                  ...
                  >
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
                        ...
                        >
                          {u.avatar_path ? (
                            <StorageImage
                              fileKey={u.avatar_path as string}
                              alt="Avatar"
                    ...
                    <td>
                      <span
                        style={{
                          color: u.is_active ? "var(--color-tertiary)" : "#B91C1C",
                          fontSize: "0.8rem",
                          fontWeight: 600,
                        }}
                      >
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.2rem" }}>
                ...
                <div
                  style={{
                    opacity: !editingUser && (modalMode !== "generic" || initialFilter) ? 0.7 : 1,
                  }}
                >
                  <label
                    style={{
                  ...
                  >
                    Rol Académico/Admin{" "}
                    {!editingUser && (modalMode !== "generic" || initialFilter) && "(Auto)"}
                  </label>
                  <select
                    name="role_id"
                    required
                    ...
                    onChange={handleFormChange}
                    disabled={Boolean(!editingUser && (modalMode !== "generic" || initialFilter))}
                    style={{
                      width: "100%",
                      padding: "0.75rem",
                      borderRadius: "10px",
                      ...
                      outline: "none",
                      background:
                        !editingUser && (modalMode !== "generic" || initialFilter)
                          ? "#f9f9f9"
                          : "white",
                    }}
                  >
            ...
            >
              ...
              >
                {selectedUser.avatar_path ? (
                  <StorageImage
                    fileKey={selectedUser.avatar_path as string}
                    alt="Avatar"
                    style={{ width: "100%", height: "100%", objectFit: "cover" }}
                  />
                ) : (
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <span
                  style={{ fontSize: "0.85rem", color: "var(--color-secondary)", fontWeight: 600 }}
                >
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: "0.85rem" }}>
                  {selectedUser.id_type} {selectedUser.id_number}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span
