...
export default function ModuleSchools() {
  ...
  const fetchSchools = useCallback(async () => {
    try {
      ...
      if (!response.ok)
        throw new Error(data.detail || "Error al descargar el catálogo educacional");
  ...
  const openEditModal = (school) => {
    ...
    setFormData({
      ...
      email: school.email || "",
      phone: school.phone || "",
      country: school.country || "PE",
      region: school.region || "Lima",
      province: school.province || "Lima",
      district: school.district || "",
      address: school.address || "",
      institution_code: school.institution_code || "",
      institution_type: school.institution_type || "private",
      founding_year: school.founding_year || new Date().getFullYear(),
      modality: school.modality || "presencial",
      language: school.language || "es",
      timezone: school.timezone || "America/Lima",
      plan_code: school.plan_code || "free",
    });
    setSelectedLogo(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let finalPayload: Record<string, unknown> = { ...formData };
      finalPayload.founding_year = parseInt(String(finalPayload.founding_year), 10) || 1990;
      ...
      if (!response.ok) {
        throw new Error(
          data.detail ? JSON.stringify(data.detail) : "Fallo de registro crudo al servidor.",
        );
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("¿Deseas suspender este Colegio moviéndolo a la papelera (Soft Delete)?"))
      return;
    try {
      const token = localStorage.getItem("token");
  ...
  const handleRestore = async (id) => {
    if (!window.confirm("¿Estás seguro de reactivar este colegio para reconectarlo al sistema?"))
      return;
    try {
      const token = localStorage.getItem("token");
              ...
              <tbody>
                {filteredSchools.map((sch) => (
                  <tr
                    key={sch.id}
                    style={{
                      opacity: activeTab === "deleted" ? 0.5 : 1,
                        ...
                        <div>
                          <div
                            style={{
                              ...
                              color: "var(--color-text)",
                              marginBottom: "0.2rem",
                              textDecoration: activeTab === "deleted" ? "line-through" : "none",
                            }}
                          >
                            {sch.name}
                          </div>
                          <div style={{ fontSize: "0.8rem", color: "var(--color-text-muted)" }}>
                            {sch.slug} | {sch.district || "NA"}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ textAlign: "right" }}>
                      {activeTab === "deleted" ? (
                        ...
                        <div style={{ display: "flex", gap: "0.4rem", justifyContent: "flex-end" }}>
                          ...
                          <button
                            onClick={() => navigate(`/dashboard/schools/${sch.id}`)}
                            style={{
                            ...
                            title="Ver Dashboard de Colegio"
                          >
                            <Eye size={16} />
                          </button>
                          <button
          ...
          >
            ...
            >
              ...
              >
                ...
                {editingSchool ? "Modificar Institución" : "Onboarding Institucional"}
              </h3>
              <button
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <fieldset
                  style={{
                    border: "1px dashed rgba(105, 151, 126, 0.5)",
                    borderRadius: "12px",
                    padding: "1rem",
                    flex: 1,
                    display: "flex",
                ...
                >
                  <legend
                    style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--color-tertiary)" }}
                  >
                    Isotipo Emblema (S3 / R2 Logo)
                  </legend>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  <div
                    ...
                    style={{
                      ...
                      height: "80px",
                      borderRadius: "50%",
                      background: logoPreview ? "white" : "rgba(45, 55, 63, 0.05)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      cursor: "pointer",
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      title="Generado automáticamente, invariable en edición."
                    />
                  </div>
                </fieldset>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                <fieldset
                  style={{
                    border: "1px solid rgba(224, 159, 57, 0.3)",
                    borderRadius: "12px",
                    padding: "1rem",
                    flex: 1,
                    background: "rgba(224, 159, 57, 0.02)",
                  }}
                >
                  <legend
                    style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--color-tertiary)" }}
                  >
                    Ecosistema Hamutay SaaS
                  </legend>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        style={{
                          ...
                          padding: "0.7rem",
                          borderRadius: "8px",
                          border: "1px solid rgba(224, 159, 57, 0.4)",
                          background: "white",
                          outline: "none",
                          fontWeight: 600,
                        }}
