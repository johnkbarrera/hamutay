...
const getResourceLabel = (slug: string) => {
...
export default function ModuleSchoolRoles() {
  ...
  const handleDeleteRole = async (roleId) => {
    if (
      !window.confirm(
        "¿Seguro de desvincular este Rol? Los administradores con este rol perderán acceso.",
      )
    )
      return;
    try {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem", minHeight: "100%" }}>
      ...
      >
        <div>
          ...
          >
            <ShieldAlert color="var(--color-primary)" /> Auditoría Global de Plantillas
          </h2>
          <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
            Modifica los accesos agnósticos del sistema. Se replicará automáticamente en los
            inquilinos.
          </p>
        </div>
      </div>
      ...
      {!loading && (
        <div
          style={{
            display: "grid",
            gridTemplateColumns: selectedRole ? "1fr 1fr" : "1fr",
            gap: "1.5rem",
            transition: "all 0.3s",
          }}
        >
                ...
                <tbody>
                  {roles.map((r) => (
                    <tr
                      ...
                      style={{
                        cursor: "pointer",
                        background:
                          selectedRole?.id === r.id ? "rgba(105, 151, 126, 0.05)" : "transparent",
                  ...
                  {roles.length === 0 && (
                    <tr>
                      ...
                      >
                        No existen roles aún. Crea el rol "Director" para empezar.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {selectedRole && (
            <div
              style={{
                background: "white",
                borderRadius: "16px",
                border: "2px solid rgba(224, 159, 57, 0.2)",
                overflow: "hidden",
                display: "flex",
            ...
            >
              <div
                style={{
                  ...
                  background: "rgba(224, 159, 57, 0.04)",
                  borderBottom: "1px solid rgba(0,0,0,0.05)",
                  display: "flex",
                  justifyContent: "space-between",
                  alignItems: "center",
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: "0.8rem", color: "var(--color-text-muted)" }}>
                    Configurando el perfil: <strong>{selectedRole.name}</strong>
                  </span>
                </div>
                {permsLoading && (
                  <Loader2
              ...
              <div style={{ padding: "1.5rem", flex: 1, overflowY: "auto" }}>
                ...
                >
                  Los permisos delimitan a qué componentes del sistema puede accceder este rol para
                  un colegio determinado. Todo aquello que no está tildado, será denegado
                  implícitamente `(HTTP 403)`.
                </p>
                        ...
                        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                          {["create", "read", "update", "delete"].map((act) => {
                            const permObj = permissions.find(
                              (p) => p.resource === resId && p.action === act,
                            );
                            ...
                            return (
                              <label
                                ...
                                style={{
                                  ...
                                  fontSize: "0.85rem",
                                  userSelect: "none",
                                  background: checked ? "rgba(105, 151, 126, 0.08)" : "transparent",
                                  padding: "0.3rem 0.6rem",
                                  borderRadius: "6px",
                                  border: checked
                                    ? "1px solid rgba(105, 151, 126, 0.3)"
                                    : "1px solid transparent",
                                  transition: "all 0.1s",
                                }}
                              >
                                {checked ? (
                                  <CheckSquare size={16} color="var(--color-tertiary)" />
                                ) : (
                                ...
                                <input
                                  type="checkbox"
                                  checked={checked}
                                  onChange={() => togglePermission(resId, act)}
                                  style={{ display: "none" }}
                                />
                              </label>
                            );
      ...
      {isRoleModalOpen && (
        ...
        >
          ...
          >
            <h3 style={{ margin: "0 0 1.5rem 0", color: "var(--color-text)" }}>
              Definir Nuevo Rol
            </h3>
