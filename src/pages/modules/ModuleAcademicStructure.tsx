  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      >
        ...
        >
          <BookOpen color="var(--color-primary)" /> Malla Curricular
        </h2>
        <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
          Administra la estructura académica global: niveles, grados, y catálogo de cursos.
        </p>
      </div>
      ...
      >
        ...
        >
          <button onClick={() => setTab("levels")} style={tabStyle(tab === "levels")}>
            Niveles
          </button>
          ...
          <button onClick={() => setTab("grade-courses")} style={tabStyle(tab === "grade-courses")}>
            Cursos por Grado
          </button>
        </div>
...
function TabLevels({ canWrite }) {
  ...
  const fetch_ = async () => {
    setLoading(true);
    try {
      const d = await apiFetch(`${API}/academic/levels`);
  ...
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar nivel "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/levels/${row.level_id}`, { method: "DELETE" });
      fetch_();
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await apiFetch(`${API}/platform/academic/levels/${editing.level_id}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch(`${API}/platform/academic/levels`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
    ...
    <>
      ...
      >
        <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.85rem" }}>
          Niveles educativos: Inicial, Primaria, Secundaria, etc.
        </p>
        {canWrite && (
...
function TabGrades({ canWrite }) {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ level_id: levels[0]?.level_id || "", name: "", order: "" });
    setModal(true);
  };
  const openEdit = (row) => {
    setEditing(row);
    setForm({ level_id: row.level_id, name: row.name, order: row.order ?? "" });
    setModal(true);
  };
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar grado "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/grades/${row.grade_id}`, { method: "DELETE" });
      fetch_();
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await apiFetch(`${API}/platform/academic/grades/${editing.grade_id}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch(`${API}/platform/academic/grades`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
...
function TabCourses({ canWrite }) {
  ...
  const fetch_ = async () => {
    setLoading(true);
    try {
      const d = await apiFetch(`${API}/academic/courses`);
      setData(Array.isArray(d) ? d : []);
    } catch {
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar curso "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/courses/${row.course_id}`, { method: "DELETE" });
      fetch_();
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await apiFetch(`${API}/platform/academic/courses/${editing.course_id}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch(`${API}/platform/academic/courses`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
    ...
    <>
      ...
      >
        <input
          ...
          style={{ ...inputStyle, width: "240px", padding: "0.5rem 0.8rem", fontSize: "0.85rem" }}
        />
        {canWrite && (
          <button onClick={openCreate} style={btnPrimary}>
...
function TabGradeCourses({ canWrite }) {
  ...
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar asignación "${row.name}"?`)) return;
    try {
      await apiFetch(`${API}/platform/academic/grade-courses/${row.grade_course_id}`, {
        method: "DELETE",
      });
      fetch_();
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await apiFetch(`${API}/platform/academic/grade-courses/${editing.grade_course_id}`, {
          method: "PUT",
          body: JSON.stringify({ name: body.name, weekly_hours: body.weekly_hours }),
        });
      } else {
        await apiFetch(`${API}/platform/academic/grade-courses`, {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
          <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "0.85rem" }}>
            Asignación de cursos a grados específicos.
          </p>
          <select
            value={filterGrade}
      ...
      <FormModal
        title={editing ? "Editar Asignación" : "Asignar Curso a Grado"}
        open={modal}
        onClose={() => setModal(false)}
        onSubmit={handleSubmit}
        submitting={submitting}
